...
PageMap.prototype = {
  _delete: function(id)
  {
    ...
    if (Object.keys(this._map).length == 0)
      delete PageMap._nonEmptyPageMaps[this._id];
  },
  get: function(page)
  {
  ...
  set: function(page, value)
  {
    ...
    PageMap._nonEmptyPageMaps[this._id] = this;
  },
  has: function(page)
  {
...
{
  for (let pageMapId in PageMap._nonEmptyPageMaps) {
    if (pageId) {
      PageMap._nonEmptyPageMaps[pageMapId]._delete(pageId);
    } else {
      PageMap._nonEmptyPageMaps[pageMapId].clear();
    }
  }
};
...
let Page = exports.Page = function(tab)
{
  ...
  if (tab.window) {
    this._id = tabUtils.getTabId(tabUtils.getTabForContentWindow(tab.window));
  }

  this.browserAction = new BrowserAction(this._id);
...
let Pages = exports.Pages = {
  ...
  query: function(info, callback)
  {
    ...
    return this.getCurrentPage(function(page){
      callback([page]);
    });
  },
...
BrowserAction.prototype = {
  ...
  {
    let {UI} = require("./ui");
    let tab = tabUtils.getTabForId(this._tabId);

    if (!badge) {
      UI.toolbarButton.state(tab, {badge: null});
      return;
    }

    if ("color" in badge) {
      UI.toolbarButton.state(tab, {badgeColor: badge.color});
    }

    if ("number" in badge) {
      UI.toolbarButton.state(tab, {badge: badge.number});
    }
  }
};
