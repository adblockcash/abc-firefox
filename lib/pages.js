...
let Pages = exports.Pages = {
  open: function(url, callback)
  {
    tabs.open({
      url: url,
      inNewWindow: true,
      onOpen: (callback ? function onOpen(openedTab) {
        openedTab.once("ready", function onReady(tab) {
  ...
  openAndListenForUrlChangesUntil: function (url, callback) {
    tabs.open({
      ...
      inNewWindow: true,
      onOpen: (callback ? function onOpen(openedTab) {
        openedTab.once("load", function onLoad(tab) {
          if (callback(tab.url) === true) {
            // openedTab.off("load", onLoad);
            openedTab.close();
          } else {
            openedTab.once("load", onLoad);
          }
        });
      } : null)
    });
