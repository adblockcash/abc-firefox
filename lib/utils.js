...
let onShutdown = exports.onShutdown =
{
  done: false,
...
{
  onMessage: new _EventTarget(),

  getURL: function(path) {
    return "chrome://adblockcash/content/" + path;
  },

  /**
   * Returns the add-on ID used by Adblock Cash
   */
  get addonID()
  {
    return info.addon.id;
  },

  /**
   * Returns the installed Adblock Cash version
   */
  get addonVersion()
  {
    return info.addon.version;
  },

  /**
   * Returns the user interface locale selected for adblockcash chrome package.
   */
  get appLocale()
  {
    let locale = "en-US";
    try
    {
      locale = Utils.chromeRegistry.getSelectedLocale("adblockcash");
    }
    catch (e)
    {
      Cu.reportError(e);
  ...
  getString: function(name)
  {
    ...
    let stringBundle = Services.strings.createBundle("chrome://adblockcash/locale/global.properties?" + Math.random());
