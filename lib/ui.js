  ...
  initOptionsDoc: function(/**Document*/ doc)
  {
    ...
    function initCheckboxes()
    {
      ...
      if (list)
      {
        ...
        request.open("GET", "chrome://adblockcash/content/ui/subscriptions.xml");
...
let UI = exports.UI =
{
  ...
  addToolbarButton: function()
  {
    ...
    if ("abc-toolbarbutton" in this.overlay && defaultToolbarPosition)
    {
      try
      {
      ...
      CustomizableUI.createWidget({
        ...
        type: "custom",
        positionAttribute: "abc-iconposition",        // For emulation only
        ...
        {
          let node = document.importNode(this.overlay["abc-toolbarbutton"], true);
      ...
      onShutdown.add(CustomizableUI.destroyWidget.bind(CustomizableUI, "abc-toolbarbutton"));
    }
  },
  ...
  applyToWindow: function(/**Window*/ window, /**Boolean*/ noDelay)
  {
    ...
    addBrowserLocationListener(window, function()
    {
      ...
      this.updateIconState(window, window.document.getElementById("abc-toolbarbutton"));
    }.bind(this));
  ...
  addSubscription: function(/**Window*/ window, /**String*/ prevVersion)
  {
    ...
    function notifyUser()
    {
      ...
      if (addTab)
      {
        addTab(window, "chrome://adblockcash/content/ui/firstRun.html");
      }
      else
      {
        let dialogSource = '\
          ...
          <dialog xmlns="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul" onload="document.title=content.document.title" buttons="accept" width="500" height="600">\
            <iframe type="content-primary" flex="1" src="chrome://adblockcash/content/ui/firstRun.html"/>\
          </dialog>';
  ...
  fillIconMenu: function(/**Event*/ event, /**Window*/ window, /**String*/ prefix)
  {
    ...
    setDisabled("abc-command-sendReport", !location || !Policy.isBlockableScheme(location) || location.scheme == "mailto");
