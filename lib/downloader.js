...
Downloader.prototype =
{
  ...
  {
    let {addon, application, applicationVersion, platform, platformVersion} = require("./info");
    ...
    if (downloadCount > 4)
      downloadCount = "4+";
    url += "addonName=" + encodeURIComponent(addon.name) +
        "&addonVersion=" + encodeURIComponent(addon.version) +
  ...
  _download: function(downloadable, redirects)
  {
    ...
    let errorCallback = function errorCallback(error)
    {
      ...
      Cu.reportError("Adblock Cash: Downloading URL " + downloadable.url + " failed (" + error + ")\n" +
