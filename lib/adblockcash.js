...
let AdblockCash = exports.AdblockCash = {
  ...
  init: function() {
    ...
    if (!this.visitor) {
      setTimeout(this.refreshWhitelistableWebsites.bind(this));
    }
  ...
  getCountriesList: function() {
    return new Promise((resolve, reject) => {
      ...
      requestPromise = this._makeApiRequest("GET", GLOBALS.ABC_BACKEND_ORIGIN + "/general/countries");
  ...
  refreshCurrentVisitor: function() {
    requestPromise = this._makeApiRequest("GET", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/account");
    requestPromise.then((response) => {
  ...
  login: function(email, password) {
    ...
    request.open("POST", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/account", true, email, password);
  ...
  updateNotificationSettings: function(window, settings) {
    return this._makeApiRequest("POST", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/notifications/settings", {
  ...
  updateVisitorAccount: function(window, visitor) {
    return this._makeApiRequest("PUT", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/account", {
        visitor: visitor
      })
      .then((response) => {
        if (response.visitor) {
  ...
  refreshWhitelistableWebsites: function() {
    ...
    return this._makeApiRequest("GET", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/websites/whitelistable")
      .then((response) => {
        if (response.websites) {
          this.whitelistableWebsites = response.websites;

          this.whitelistableWebsites
            .filter(function(website){
              return !this.isWhitelistableDomainBlocked(website.domain);
            }.bind(this))
            .forEach(function(website){
              var filterText = "@@||" + website.domain + "^$document";
  ...
  // When we unwhitelisted a whitelistable domain,
  // block it - so it won't be automatically added as whitelisted
  // when we refresh whitelistableWebsites.
  blockWhitelistableDomain: function(domain) {
    Prefs.blockedWhitelistableDomains.push(domain);
    Prefs.blockedWhitelistableDomains = Prefs.blockedWhitelistableDomains;
  },

  // Invert the effect of this.blockWhitelistableDomain()
  unblockWhitelistableDomain: function(domain) {
    var i = Prefs.blockedWhitelistableDomains.indexOf(domain);
    if (i) {
      Prefs.blockedWhitelistableDomains.splice(i, 1);
    }
    Prefs.blockedWhitelistableDomains = Prefs.blockedWhitelistableDomains;
  },

  isWhitelistableDomainBlocked: function(domain) {
    return Prefs.blockedWhitelistableDomains.indexOf(domain) >= 0;
  },
  ...
  sendVisitDetection: function(domain, whitelisted) {
    ...
    function debouncedFunction(domain, whitelisted) {
      return this._makeApiRequest("POST", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/detector/visit", {
