...
let AdblockCash = exports.AdblockCash = {
  ...
  getCountriesList: function() {
    return new Promise((resolve, reject) => {
      ...
      requestPromise = this._makeApiRequest("GET", GLOBALS.ABC_BACKEND_ORIGIN + "/general/countries.json");
  ...
  sendVisitDetection: function(domain, whitelisted) {
    ...
    function debouncedFunction(domain, whitelisted) {
      if (this._isOtherAdblockEnabled) {
        return false;
      }
    ...
    if (chrome) {
      ...
      return new Promise((resolve, reject) => {
        chrome.management.getAll((extensions) => {
          ...
          let isOtherAdblockEnabled = this._isOtherAdblockEnabled = enabledAdblocks.length > 0;
