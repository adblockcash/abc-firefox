...
let AdblockCash = exports.AdblockCash = {
  ...
  refreshCashableWebsites: function() {
    ...
    return this._makeApiRequest("GET", GLOBALS.ABC_BACKEND_ORIGIN + "/visitor/websites/cashable")
      .then((response) => {
        if (response.websites) {
          ...
          this.cashableWebsites
            .filter((website) => {
              return !this.isCashableDomainBlocked(website.domain);
            })
            .forEach((website) => {
              this.addWhitelistedDomain(website.domain, false, true);
            });
        } else {
  ...
  addWhitelistedDomain: function(domain, blockCashableDomain, silent) {
    ...
    return FilterStorage.addFilter(Filter.fromText(filterText), silent);
  },
  ...
  removeWhitelistedDomain: function(domain, blockCashableDomain, silent) {
    ...
    FilterStorage.removeFilter(Filter.fromText("@@||" + domain + "^$document"), silent);
  },
