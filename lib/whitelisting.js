...
let {PageMap} = require("./pages");
let UriUtils = require("./utilsUri");

let pagesWithKey = new PageMap();

let isWhitelisted = exports.isWhitelisted = function(url, parentUrl, type, key)
{
  let filter = defaultMatcher.matchesAny(
    UriUtils.stripFragmentFromURL(url),
    type || "DOCUMENT",
    UriUtils.extractHostFromURL(parentUrl || url),
    false,
    key
  );
...
{
  let url = UriUtils.stripFragmentFromURL(frame.url);
  let docDomain = UriUtils.extractHostFromURL((frame.parent || frame).url);
