...
let FilterStorage = exports.FilterStorage =
{
  ...
  get sourceFile()
  {
    ...
    if (!file)
    {
      ...
      try
      {
        file = IO.resolveFilePath(Services.prefs.getDefaultBranch("extensions.adblockcash.").getCharPref("data_directory"));
        if (file)
          file.append("patterns.ini");
      } catch(e) {}
    }
    ...
    if (!file)
      Cu.reportError("Adblock Cash: Failed to resolve filter file location from extensions.adblockcash.patternsfile preference");
   ...
   * @param {Boolean} silent  if true, no listeners will be triggered (to be used when filter list is reloaded)
   */
  removeFilter: function(filter, subscription, position, silent)
  {
  ...
  increaseHitCount: function(filter, wnd)
  {
    ...
    filter.setHitCount(filter.hitCount + 1, wnd);
    filter.setLastHit(Date.now(), wnd);
  },
  ...
  {
    yield "# Adblock Cash preferences";
